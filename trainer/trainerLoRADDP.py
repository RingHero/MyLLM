...
def train():
    ...
    # 应用lora
    apply_lora(model, lora_rank=4)
    # 冻结除LoRA层以外的所有参数, 收集LoRA层参数
    lora_params = []
    ...
    # 学习率调小，防止破坏基座常识
    max_lr = 5e-5 
    optimizer = optim.AdamW(lora_params, lr=max_lr, weight_decay=0.1)
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if global_step % save_step == 0 or global_step == MAX_TRAIN_STEPS:
                ...
                if rank == 0:
                    ...
                    torch.save(checkpoint, f'/home/apulis-dev/userdata/tmp/sft_lora_step_{global_step}.pt')
